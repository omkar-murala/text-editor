...
//     // Broadcast the list of connected users to everyone in the room except the new user
//     io.in(roomId).emit('connected-users', roomUsers[roomId].map(user => user.username));
...
//     // Notify other users in the room
//     socket.to(roomId).emit('user-left', username);
//     // Update the list of connected users in current room
//     io.in(roomId).emit('connected-users', roomUsers[roomId].map(user => user.username));
//   });
// });

// app.get('/', (req, res) => {
//   res.send('Welcome to the Realtime Collaboration App!');
// });
